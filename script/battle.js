(function () {
  ...
  window.BattleSystem = {
    start(level) {
      ...
      if (!enemy) return { finished: true, message: "Ukonczyles wszystkie dostepne poziomy gry!" };
    ...
    attack(state) {
      ...
      if (roll100() < finalDodge) {
        return { ...state, enemyDefeated: false, damage: 0, heal: 0, message: "Przeciwnik uniknal twojego ataku!" };
      }
      ...
      return { ...state, enemyHealth: nextHealth, enemyDefeated: nextHealth <= 0, damage, heal, message: `Zadales ${damage} obrazen!` };
    },
    ...
    enemyTurn(state) {
      ...
      if (roll100() < enemy.attackChance) {
        ...
        return { ...state, enemyHit: true, message: `Przeciwnik zadal Ci ${enemy.damage} obrazen!` };
      }
    ...
    nextWave(state) {
      ...
      const nextState = { ...state, currentWave: nextWave, enemyHealth: enemy.health, levelUp: false, reward: enemy.reward, message: "Przeciwnik pokonany! Nadchodzi nastepna fala." };
    ...
    escape() {
      const player = window.player;
      if (player.usedEscape) return { allowed: false, message: "Mozesz uciec tylko raz na poziom!" };
